    ...
    def generate2(
        ...
        **kwargs
        ...
        if torch.is_tensor(tokenized_sentences) and tokenized_sentences.dim() == 1:
            ...
            output,pre_scores = self.generate2(
                tokenized_sentences.unsqueeze(0), beam=beam, prefix_tokens = prefix_tokens, attn_len = attn_len, verbose=verbose, **kwargs
            )

            return output[0],pre_scores[0]
        ...
        for batch in self._build_batches(tokenized_sentences, skip_invalid_size_inputs):
            ...
            translations, pre_scores = self.task.inference_step2(
                generator, self.models, batch, **inference_step_args
            )
            for id, hypos in zip(batch["id"].tolist(), translations):
                results.append((id, hypos))

            all_pre_scores.append(pre_scores)
